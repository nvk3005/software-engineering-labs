...
export default function OrderDetailPage() {
  ...
  const submitCancel = async () => {
    if (!order) return;
    const result = await dispatch(cancelOrder({ orderId: order.id, reason }));
    if (cancelOrder.fulfilled.match(result) && typeof result.payload?.loyalty?.currentPoints === "number") {
      dispatch(setLoyaltyPoints(result.payload.loyalty.currentPoints));
    }
  };
